...
| 재요청 시 동작 | 2026-08-26부터 **항상 새로 수집** — 같은 날짜를 재요청해도 예전 파일을 재사용하지 않고 업비트를 다시 불러 덮어씀(`collectAllMarkets` → `collectMarket(forceFresh=true)`). 트레이더가 부르는 온디맨드 보충 경로(`ensureMarketCollected`)는 여전히 기존 파일이 있으면 재사용하는 멱등 동작을 유지함 |
