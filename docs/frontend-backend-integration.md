...
- **총 재생 예정 건수 + 예상 소요 시간 + 예상 평균 TPS** → `orderapi`의 `GET /v1/jobs/replay-preview?date=YYYY-MM-DD` (4.8 참고). `totalOrders`는 그날 20개 마켓 전체의 기록된 주문 수 합. `maxEventSpanSeconds`는 배속 1일 때 값이라, 프론트가 재생 시작 폼에서 이미 고른 speed로 **나눠서** 예상 소요 시간을 계산하면 된다(`replayengine`은 마켓마다 독립된 고루틴으로 동시에 재생하므로, 전체 실행 시간은 각 마켓 소요 시간의 **합이 아니라 가장 긴 마켓 하나에 수렴** — 그래서 서버가 마켓별 최댓값을 미리 계산해서 준다). 기록이 없는 마켓은 조용히 건너뛴 값이라 별도 처리 필요 없음. **예상 평균 TPS(2026-08-20)**도 같은 두 값 + speed로 프론트에서 바로 계산 가능(`totalOrders × speed ÷ maxEventSpanSeconds`, 4.8 참고) — 새 API 호출 없이 실행 전에 "대략 초당 몇 건 정도로 부하가 들어갈지"를 미리 보여줄 수 있다. AI 트레이더 화면엔 적용 불가(4.8 참고, 봇이 즉석에서 주문을 만들어 총 건수를 미리 알 수 없음).
...
- **재생 가능한 날짜 목록 → `GET /v1/jobs/replay-dates` (2026-08-26 신설, 팀 요청).** 지금까지는 날짜 선택 필드가 자유 입력이라, 트레이더가 그 날짜에 기록해둔 게 아예 없어도(=`replay-preview`가 `totalOrders: 0`을 줄 뿐 막지는 않음) 프론트가 그 날짜로 재생을 시작할 수 있었다. 이 엔드포인트는 `{"dates": ["2026-08-25", "2026-08-19", ...]}` 형태로 **주문 기록 파일이 실제로 하나라도 있는 날짜만, 최신순으로** 돌려준다 — 날짜 선택 UI(달력/드롭다운)를 이 목록으로 제한하면, "기록 없는 날짜를 골라서 재생했는데 아무 일도 안 일어남" 상황 자체를 차단할 수 있다. 마켓 하나만 있어도 그 날짜는 포함된다(20개 마켓이 전부 있어야 하는 게 아님 — 리플레이 자체가 기록 없는 마켓은 건너뛰고 진행하므로). 기록이 하나도 없으면 빈 배열(`404`가 아니라 `200 {"dates": []}`). 파라미터 없음.
...
```
`speed`는 이 실행이 시작될 때 쓴 `-speed` 값이다(0이면 필드 자체가 응답에서 빠짐 — `trader`/`replayengine`이 `POST /v1/sessions` 호출 시 보내는 값을 그대로 기록한 것). 다른 숫자(특히 미체결 건수)를 해석하는 데 필요한 맥락이라 같이 내려준다 — 배속에 따라 같은 미체결 건수도 의미가 완전히 다르다. 한 번도 실행된 적 없으면 404 `{"errorCode":"NO_RUN_YET", ...}`.

**`message`엔 세션 종료 시 자동 정리가 건너뛰어졌다는 안내가 덧붙을 수 있다(2026-08-20).** 세션이 끝나는 순간 매칭 엔진이 컨슈머 랙 상태였으면, 자동 정리(4.11의 수동 정리와 같은 로직, 세션 종료 시 자동으로 도는 버전)가 아무것도 안 하고 건너뛰면서 `message`에 `"미종결 주문 자동 정리가 매칭 엔진 컨슈머 랙으로 건너뛰어짐 — 매칭 엔진 회복 후 수동 일괄 정리(POST /v1/admin/cleanup-unresolved-orders) 필요"`가 (기존 메시지가 있으면 ` | `로 이어붙여) 추가된다. 프론트는 이 문구를 그냥 텍스트로 보여줘도 되고, 특정 패턴을 감지해서 "일괄 정리" 버튼(4.11)을 강조 표시하는 것도 고려할 수 있다 — 다만 문구 자체를 파싱하기보다는 그냥 사람이 읽을 안내로 취급하는 걸 권장(내부적으로 바뀔 수 있음). 실행 도중 크래시(정상 종료 경로를 못 타서 `trader`/`replayengine`이 결과를 못 남긴 경우)는 `status`가 계속 `IN_PROGRESS`로 남아있을 수 있다 — 이 값은 세션 자체의 배타적 잠금(TTL 30초)과는 별개로 영구 보관되는 기록이라 자동으로 안 바뀐다. 실제 로컬 Redis로 클레임 직후(`IN_PROGRESS`) → `DELETE /v1/sessions/{id}`로 `COMPLETED`/`FAILED` 반납 → 이 값이 정확히 반영되는 것까지 확인함.
...
**예상 평균 TPS도 이 응답만으로 프론트에서 바로 계산 가능(2026-08-20, 백엔드 변경 없음)**:
```
평균 TPS = totalOrders × speed ÷ maxEventSpanSeconds
```
`maxEventSpanSeconds / speed`가 실제 걸릴 시간(초)이니, 그걸로 `totalOrders`를 나누면 된다 — 예상 소요 시간 계산과 같은 두 값(`totalOrders`, `maxEventSpanSeconds`)에 이미 폼에 있는 `speed`만 곱하면 되므로 새 API 호출이나 백엔드 필드 추가가 필요 없다. **AI 트레이더(페이퍼 트레이딩) 화면엔 이 계산을 적용할 수 없다** — 봇이 실행 중에 즉석으로 주문을 판단해서 만들기 때문에 시작 전엔 총 주문 수 자체를 알 수 없다(리플레이는 `trader`가 이미 기록해둔 파일을 재생하는 거라 총 주문 수가 시작 전에 이미 확정돼 있다는 점이 다르다).
...
- **2026-08-20에 프론트 계산 가이드 추가(백엔드 변경 없음)**: "부하 시나리오 미리보기" 화면이 이미 갖고 있는 `totalOrders`/`maxEventSpanSeconds`(4.8) + 폼의 `speed`만으로 예상 평균 TPS를 계산할 수 있다는 공식(`totalOrders × speed ÷ maxEventSpanSeconds`)을 문서화(3.6/4.8 참고) — 새 API나 필드가 필요 없어 순수 프론트 작업.
