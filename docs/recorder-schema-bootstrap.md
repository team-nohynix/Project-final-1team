...
## 2026-08-20: 인덱스 3개 추가 — 실 서비스 RDS에 적용 필요 (시급)

같은 날 CPU 병목을 조사하면서 원인이 두 겹으로 겹쳐 있었다는 게 드러났다:

1. **`idx_trade_order_mode_submitted ON trade_order (mode, submitted_at, status)`** — `recorder/query.OrderSummary`/`UnresolvedOrders`(세션 종료 시 미종결 주문 자동 정리, `CLAUDE.md`의 "Session-end unresolved-order cleanup" 참고)가 커버 인덱스 없이 매번 풀스캔하고 있었다. 세션 정리가 세션 종료마다 자동으로 도는 기능이 되면서(`RECORDER_URL` 배선 완료, 2026-08-19/20) 이 풀스캔이 반복 트리거돼 **RDS 라이터 인스턴스 CPU가 13:09 KST부터 계속 ~99%에 붙어있는 실제 장애**로 이어졌다(`aws cloudwatch get-metric-statistics --namespace AWS/RDS --metric-name CPUUtilization --dimensions Name=DBInstanceIdentifier,Value=team1-truss-db-instance-1`로 확인 — Aurora는 `DBClusterIdentifier`로 조회하면 데이터가 안 나오고, 라이터 인스턴스의 `DBInstanceIdentifier`로 조회해야 함).
2. **`idx_trade_order_submitted_at ON trade_order (submitted_at)` / `idx_execution_executed_at ON execution (executed_at)`** — 위 인덱스를 적용한 뒤에도 CPU가 57~65%에서 30분 넘게 안 내려가서 더 파보니, 이번엔 recorder 자신의 `pollDashboardMetrics`(Grafana 게이지 갱신용으로 10초마다 영원히 도는 폴러, `recorder/metrics.go`)가 원인이었다. 이 폴러의 TPS/시계열 쿼리가 `submitted_at`/`executed_at`을 단독으로(다른 컬럼 없이) 필터링하는데, 이 컬럼이 선행 컬럼인 인덱스가 하나도 없어서 220만 행짜리 테이블을 10초마다 통째로 훑고 있었다. `WriteIOPS`가 이 구간 내내 거의 0이었던 것으로(recorder가 쓰기 백로그를 처리 중이던 게 아니라 순수 읽기 폴링이 원인이었음을) 확인.

`idx_trade_order_status`(Jaden Yang이 직접 적용, 아래 참고)를 빼면 나머지 3개는 아직 실 RDS에 적용 안 된 상태다.

`recorder/schema.sql`이 이미 다 고쳐져 있고(위 스키마 적용 방식 그대로 재실행하면 됨), 재실행해도 안전하다(디스포저블 Docker MySQL로 첫 적용/재적용 둘 다 검증 완료). **실 RDS에 이 파일을 다시 적용하는 것만 남았다** — 아래 명령을 그대로 실행하면 된다(같은 `<recorder DB 이름>`으로). 새로 생기는 테이블/기존 인덱스는 전부 건드리지 않고 아직 없는 인덱스만 새로 추가된다.

(참고: 같은 날 추가된 `idx_trade_order_status`는 Jaden Yang이 이미 실 RDS에 직접 무중단 적용을 끝내둔 상태라, 이 재적용에서는 `create_index_if_absent`가 "이미 있음"으로 보고 건너뛴다.)
...
# 3. 확인 — 새 인덱스 3개가 다 생겼는지
kubectl run mysql-client --rm -i --restart=Never --namespace=backend --image=mysql:8.4 \
  --command -- mysql -h team1-truss-db.cluster-cnqmcq6uwqa3.ap-northeast-2.rds.amazonaws.com \
  -u "$DB_USER" -p"$DB_PASS" <recorder DB 이름> \
  -e "SHOW INDEX FROM trade_order WHERE Key_name IN ('idx_trade_order_mode_submitted','idx_trade_order_submitted_at'); SHOW INDEX FROM execution WHERE Key_name='idx_execution_executed_at';"
```
