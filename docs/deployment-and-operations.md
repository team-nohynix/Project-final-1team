...
| team1-recorder | c6i.xlarge 고정 | recorder 전용 (2026-08-25 분리) | 8 vCPU (c6i.xlarge 최대 2대 상당) |

recorder가 60배속 세션 종료 시점 체결 배치를 처리하려면 c6i.large의 allocatable을 거의 다 쓰는 2.5Gi가 필요해, recorder만 별도 NodePool(c6i.xlarge, 5Gi)로 분리했다(2026-08-25). 두 NodePool 모두 `consolidateAfter`를 2026-08-26 5분→2분으로 단축(스케일다운 체감 속도 개선).
...
| matching-engine | sum(matching_engine_book_size) | 20000 | 랙은 0인데 미체결 주문이 메모리에 쌓여 OOM되는 상황을 랙 지표가 못 잡아서 추가(2026-08-21) |
...
| 날짜 | 변경 | 이유 |
|---|---|---|
| 2026-08-20 | matching-engine minReplicaCount: 1 → 2 | 버스트마다 단일 파드가 20개 마켓 물량을 스케일업(15~90초) 동안 혼자 받다 반복 OOM → 컨슈머 그룹 리밸런스로 랙/백프레셔 전파. "혼자 다 받는 상황" 자체를 제거 |
| 2026-08-21 | matching_engine_book_size 트리거 추가 (임계값 20000) | 랙 0에서도 호가창에 미체결 주문이 무기한 쌓이며 OOM — 컨슈머 랙만으로는 이 압박을 못 봄 |
| 2026-08-26 | matching-engine scaleDown stabilizationWindow: 300초 → 60초 | minReplicaCount=2 바닥이 이미 재발을 막아주므로, 5분씩 안전 마진을 둘 필요가 줄었다고 판단해 단축 |
| 2026-08-25 | recorder 전용 NodePool(team1-recorder) 분리, 메모리 한도 2.5Gi → 5Gi | 60배속 세션 종료 시점 체결 배치 처리 중 OOMKilled 재발(512Mi→1Gi로도 재현) → 안정화 확인 후 반영 |
| 2026-08-26 | recorder NodePool consolidateAfter: 5분 → 2분 | 스케일다운 체감 속도 개선 요청 — team1-backend와 동일 이유 |
...
전부 SNS(team1-sns-alerts) 하나로 발행된다. alb_5xx/redis_cpu 모두 처음엔 아직 존재하지 않는 리소스를 기준으로 차원을 잡아둬 사실상 한 번도 안 울리는 상태였다가, 실제 리소스가 생긴 뒤 정정했다(2026-08-12, 2026-08-25).

## 4. 운영 절차 (런북)

### 4.1 야간 비용 절감 destroy → 익일 복구

비용 절감을 위해 밤에 EKS 클러스터를 통째로 지웠다가 다음 날 다시 올리는 걸 반복 검증했다(2026-08-24~25) — 복구는 사람이 수동으로 트리거하며 인프라부터 K8s 리소스까지 자동으로 재구성된다. MySQL은 RDS가 아니라 EC2+EBS라 destroy 시 데이터가 스냅샷 없이 사라진다 — 디스포저블 테스트 데이터라 통상 문제는 없지만, 보존이 필요하면 별도 백업 절차가 필요하다.

### 4.2 MySQL 스키마 적용

recorder는 스키마를 자동 마이그레이션하지 않는다(마이그레이션 툴 없음, 저장소 컨벤션). 2026-08-21 RDS를 삭제하고 자체 호스팅 MySQL EC2로 전환하면서 recorder/schema.sql을 컨테이너의 `/docker-entrypoint-initdb.d/`에 올려둬 최초 기동 시 자동 적용되도록 했다 — `CREATE TABLE IF NOT EXISTS` / `CALL create_index_if_absent(...)`로만 되어 있어 재실행해도 안전하다(멱등, 검증 완료).
