...
// useIAM이 false면 nil을 돌려줍니다 — kafka.ReaderConfig.Dialer가 nil이면 인증
// 없는 기본 연결을 그대로 쓰므로, 로컬 dev-kafka(SASL 없음)에서는 아무것도 안
// 바뀝니다. true면 AWS_MSK_IAM(SASL/OAUTHBEARER)+TLS로 인증합니다 — MSK
// Serverless는 IAM 인증만 지원하고 SASL/SCRAM 자체를 지원하지 않는다는 걸
