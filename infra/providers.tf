...
# kubernetes/helm 프로바이더 인증 — recorder-db-secret(kubernetes_secret)과 karpenter/
# aws-load-balancer-controller/keda/kube-state-metrics/node-exporter(helm_release)가
...
# 403 forbidden으로 실패한다(AWS eventual-consistency). 모든 kubernetes_*/
# helm_release 리소스가 이 리소스에 depends_on을 걸어 클러스터가 ACTIVE된 뒤
# 60초를 더 기다리게 한다.
